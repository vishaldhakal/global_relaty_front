...
export const fetchAllBlogPosts = async () => {
  const res = await fetch("https://api.globalhomes.ca/api/news/", {
    next: { revalidate: 10 },
  });
...
export const fetchBlogPostBySlug = async (slug) => {
  const res = await fetch(`https://api.globalhomes.ca/api/news/${slug}`, {
    next: { revalidate: 10 },
  });
...
export const fetchBlogPostByCity = async (citySlug) => {
  const res = await fetch(
    `https://api.globalhomes.ca/api/news/?city=${citySlug}`,
    {
      next: { revalidate: 10 },
    }
  );
